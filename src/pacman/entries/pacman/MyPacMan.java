...
public class MyPacMan extends Controller<Constants.MOVE> {
    ...
    public Constants.MOVE getMove(Game game, long timeDue) {
        Constants.MOVE move = decisionTreeRoot.getLeaf(new DataTuple(game, Constants.MOVE.NEUTRAL)).getMove();
        return move;
    }

    private TreeNode createNewTree() {
        LinkedList<String> attributes = Utils.getAttributes();
    ...
    private TreeNode generateTree(LinkedList<DataTuple> data, LinkedList<String> atr) {
        ...
        if (data.size() > 0 && Utils.allTupleSameClass(data)) {
            ...
            atr.remove(bestAtr);
            String[] atrValue = Utils.getAttributeValue(bestAtr);
            for (String value : atrValue) {
                //Seperate all tuples in D so that attribute A takes the value
                LinkedList<DataTuple> subset = Utils.getSubset(data,bestAtr,value);
                if(subset.isEmpty()){
                    TreeNode child = new TreeNode();
                    Constants.MOVE move = Utils.getMajorityClass(data);
